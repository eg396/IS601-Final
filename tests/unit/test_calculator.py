...
from app.operations import add, modulo, subtract, multiply, divide  # Import the calculator functions from the operations module
...
@pytest.mark.parametrize(
    "a, b, expected",
    [
        (9, 4, 1.0),           # Test modulus two positive integers
        (-6, 3, 0.0),         # Test modulus a negative integer by a positive integer
        (18.0, 4.0, 2.0),       # Test modulus two positive floats
        (-6.0, 4.0, 2.0),     # Test modulus a negative float by a positive float
        (0, 5, 0.0),            # Test modulus zero by a positive integer
    ],
    ids=[
        "modulo_two_positive_integers",
        "modulo_negative_integer_by_positive_integer",
        "modulo_two_positive_floats",
        "modulo_negative_float_by_positive_float",
        "modulo_zero_by_positive_integer",
    ]
)
def test_modulo(a: Number, b: Number, expected: float) -> None:
    """
    Test the 'modulo' function with various combinations of integers and floats.

    This parameterized test verifies that the 'modulo' function correctly gets the remainder from dividing the first
    number by the second, handling both positive and negative values, as well as integers
    and floats. Parameterization allows for efficient and comprehensive testing across multiple cases.
    ...
    Steps:
    1. Call the 'modulo' function with arguments 'a' and 'b'.
    2. Assert that the result is equal to 'expected'.
    ...
    >>> test_modulo(-6, 4, 2.0)
    """
    # Call the 'modulo' function with the provided arguments
    result = modulo(a, b)
    
    # Assert that the result of modulo(a, b) matches the expected value
    assert result == expected, f"Expected divide({a}, {b}) to be {expected}, but got {result}"


# ---------------------------------------------
# Negative Test Case: Modulo Division by Zero
# ---------------------------------------------
...
def test_modulo_by_zero() -> None:
    ...
    Steps:
    1. Attempt to call the 'modulo' function with arguments 6 and 0, which should raise a ValueError.
    2. Use pytest's 'raises' context manager to catch the expected exception.
    3. Assert that the error message contains "Cannot divide by zero!".
